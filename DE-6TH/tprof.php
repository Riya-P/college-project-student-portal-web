...
<?php 
		...
		while($row = mysqli_fetch_array($result)){
			$prof_sub_name=$row['sub_name'];
		}	

		if(isset($_POST['MPDF']))
		{
			header("location: marks_pdf.php?prof_id=".$prof_id);
		}		
?>
<?php
	// $db=mysqli_connect('localhost','root','','de_6') or die("DB could not fount");

	$qe="SELECT stu_id,stu_name,mid1_marks FROM marks_mid_6it WHERE sub_name='$prof_sub_name'";
	$re=$db->query($qe);
	...
	{
		$filename = "$prof_sub_name _Mid1".".xls";
...
<!--------------------------------------------------Home page---------------------------------------------------------->
	...
	<div id="home" class="profmain" >
		<center>
			<h2><u>Professor Time-Table</u></h2>
		<!--------For RavindraSir AJ----------------->
		<?php 
			if($prof_id=="102"){
		?>
		...
		<form method="post" style="margin-top: 15px">
			...
			<?php
				...
				{		
					...
					$setotal=0;$yse=0;$nse=0;$ajtotal=0;$yaj=0;$naj=0;$dcdrtotal=0;$ydcdr=0;$ndcdr=0;$nettotal=0;$ynet=0;$nnet=0;$wttotal=0;$ywt=0;$nwt=0;$net=0;$aj=0;$se=0;$dcdr=0;$wt=0;$fsub=NULL;
...
<!----------------------------------------Take Attendance--------------------------------------------------------------->
	...
	<div id="takeAttendance" class="profmain">
		...
		<h4>Subject: <?php echo $prof_sub_name;?></h4>
				...
				elseif($sub=='WT-6IT'){
					if(isset($_POST['TakeAtten'])){
						...
						if($result->num_rows>0){
							while($check=$result->fetch_assoc()){
								if($wt_6it_date==$check['wt_6it_date']){
									$b=true;?>
									<p style="background-color: red;color: white"><?php echo "Attendance already taken.";?></p>
								<?php }
							}
						}
						...
						if(!$b){
							...
							if($inserResult){?>
								<p style="background-color: lightgreen"><?php echo "Attendance taken successfuly";?></p>
							<?php }
						}
		...
		<input type="submit" value="Take Attendance" class="btn" name="TakeAtten" style="width: 20%; margin-left:0; margin-top: 4%">
	</form>
...
<div id="EmidSem" class="profmain">
	<form method="post">
				...
				<?php
					...
					<tr>
						...
						<td style="width: 100px"><input type="text" name="atend[<?php echo $show['stu_id']?>]" style="width: 50px" placeholder="<?php  
											$query12="SELECT * FROM marks_mid_6it WHERE stu_id='$edstu_id' AND sub_name='$prof_sub_name'";
											$result12=$db->query($query12);
											while($show12=$result12->fetch_assoc())
											{
		...
		<div style="display: inline;">
 		...
 		<!--------------------Generate Report---------------------->
 			<form method="post" action="marks_pdf.php">
 				<a href="marks_pdf.php" target="_blank"><input type="submit"  value="Download as PDF" name="submit" class="btn" style="width: 200px;margin-left: 170px"></a>
 			</form>
		</div>
</div>
...
<div id="gtu" class="profmain">
	<center>
		...
		<tbody>
				...
				<?php
					$sr=0;
					$qu="SELECT * FROM marks_mid_6it WHERE sub_name='$prof_sub_name'";
					$re=$db->query($qu);
